...
class MlLayer:
    ...
    def ml_filter_and_sort_distractful(self, filter_threshold, df):
        ...
        logging.info('Sorting and filtering using threshold: '+str(filter_threshold))
    ...
    def ml_retrain(self, data_layer):
        ...
        if len(data_layer.dataset_feedbacks) < Params.NUM_OF_TRAINING_SAMPLES:
            # Not enough feedbacks, fill the rest from kaggle dataset
            ...
            # Choose sample from feedbacks
            df = data_layer.dataset_feedbacks.sample(n=Params.NUM_OF_TRAINING_SAMPLES)
          
        logging.info('Total training sample: {}; total feedbacks: {}'.format(Params.NUM_OF_TRAINING_SAMPLES, len(data_layer.dataset_feedbacks)))
        ...
        logging.info('Retraining finished, storing model')
